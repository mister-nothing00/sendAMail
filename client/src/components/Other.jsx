import React from 'react'
import "./component.style.css";

function Other() {
  return (
    <>
     <div id="other">
      <div className="container-other">
        <div className="other-text">
          <h3>This is a headline</h3>
          <p>
            Labore excepteur Lorem magna nostrud. Sunt ullamco id duis et et
            dolore culpa voluptate veniam aliqua. Commodo dolore consequat
            laborum ea. Consectetur do ad sunt exercitation et excepteur
            aliqua.
          </p>
        </div>
        <div className="other-image">
          <img
            src="data:image/png;base64,"
            alt="About"
            title="About Image"
          />
        </div>
      </div>
    </div>
    </>
  )
}

export default Other